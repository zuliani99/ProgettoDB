...
def statisticsHome():
	...
	guadagniTotali = conn.execute("SELECT IFNULL(sum(prenotazioni.prezzo_bagaglio)+ sum(voli.prezzo),0) FROM prenotazioni JOIN voli ON prenotazioni.id_volo = voli.id").fetchone()
	...
	infoVoli = conn.execute(
		...
		"GROUP BY voli.id HAVING voli.dataOraArrivo <= CURRENT_DATE"
	...
	conn.close()
	return render_template('statistiche.html', title='Statistiche', totPasseggeri = totPasseggeri[0], totPasseggeriMese = totPasseggeri_mese[0], aeroporti = infoAeroporti, guadagniTot = guadagniTotali[0], trattaGuadagniMax = trattaGuadagniMax, voli=infoVoli)
